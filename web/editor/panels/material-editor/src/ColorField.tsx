...
//
// Color picker for linear-RGB float triples. The engine stores albedo
// and emission in linear-RGB; the swatch and hex code in the UI live
// in sRGB display space, so we transparently round-trip:
//
//   prop (linear float) --> sRGB display --> user edit --> linear float
//
// The popover is a minimal Saturation/Value plane + Hue bar + numeric
// readouts. Emission can exceed [0,1] (W/sr); we render the swatch
// clamped to [0,1] but the numeric inputs accept any positive float.
...
export interface ColorFieldProps {
  // Linear-RGB triple (engine-side).
  value: [number, number, number];
  onCommit: (rgb: [number, number, number]) => void;
  onScrub?: (rgb: [number, number, number]) => void;
  // Allow HDR-range values (emission). Hex display + sRGB conversion
  // clamp into [0,1] for the visual swatch but the underlying value
...
// HSV <-> sRGB. We pick / drag in HSV space (intuitive) and round-trip
// through sRGB before converting back to engine-side linear floats.
...
export function ColorField({
  ...
  // Local working color in linear-RGB so the popover can drag without
  // dispatching on every move (we throttle via onScrub).
  const [draft, setDraft] = useState<[number, number, number]>(value);

  // Hex input mirror, kept as text so the user can backspace freely.
  const [hexText, setHexText] = useState<string>(() => rgbToHex(value));
  const editingHexRef = useRef<boolean>(false);

  // For HDR emission, the user wants a brightness multiplier; we
  // factor it out so the swatch / hue / SV plane operates in [0,1].
  // multiplier = max(value). If HDR allowed AND max > 1 we display
  // the normalised color in the picker.
  const hdrMultiplier = useMemo(() => {
    if (!allowHdr) return 1;
    return Math.max(1, Math.max(draft[0], draft[1], draft[2]));
  }, [allowHdr, draft]);

  // Normalised (display) color = draft / hdrMultiplier.
  const display = useMemo<[number, number, number]>(() => {
    return [draft[0] / hdrMultiplier, draft[1] / hdrMultiplier, draft[2] / hdrMultiplier];
  }, [draft, hdrMultiplier]);

  // HSV derived from sRGB(display).
  ...
  // Push prop changes into draft / hex while the popover is closed
  // and the hex input isn't actively being edited.
  useEffect(() => {
    if (open) return;
    setDraft(value);
  ...
  // Always reflect draft -> hex unless the hex field has focus.
  useEffect(() => {
  ...
  const onSwatchClick = useCallback(() => {
    ...
    if (rect) {
      // Anchor the popover below-right of the swatch; the panel often
      // lives on a narrow window so we keep it inside.
  ...
  useEffect(() => {
    ...
    const onDocPointerDown = (ev: PointerEvent) => {
      ...
      if (!target) return;
      if (target.closest('.insp-popover')) return;
      if (target.closest('.insp-swatch')) return;
      closePopover();
    };
  ...
  const onHexBlur = useCallback(() => {
    ...
    if (parsed) {
      // Re-scale by HDR multiplier so brightness is preserved.
  ...
  // Numeric RGB inputs (linear-float, with optional HDR range).
  ...
  // CSS for the SV plane uses hue-only background, with white & black
  // gradients overlaid; same trick as react-colorful.
  ...
  return (
    <>
      <div className="insp-color">
        ...
        >
          <div className="insp-swatch-fill" style={{ background: swatchFill }} />
        </div>
        <input
          className="insp-hex"
          type="text"
      ...
      {open && anchor && (
        <div
          className="insp-popover"
          style={{ left: anchor.x, top: anchor.y }}
        >
          <div
            ref={svPlaneRef}
            className="insp-sv-plane"
            style={{ background: svBackground }}
            onPointerDown={onSvPointerDown}
          >
            <div
              className="insp-sv-cursor"
              style={{
                left: `${hsv[1] * 100}%`,
                top: `${(1 - hsv[2]) * 100}%`,
              }}
          ...
          <div
            ref={hueBarRef}
            className="insp-hue-bar"
            onPointerDown={onHuePointerDown}
          >
            <div
              className="insp-hue-cursor"
              style={{ left: `${(hsv[0] / 360) * 100}%` }}
            />
          </div>
          <div className="insp-rgb-grid">
            <label>
              R
              <input
                type="text"
