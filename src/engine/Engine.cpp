...
void Engine::RenderFrame() {
    ...
    // --- end SDF Phase 1 ---------------------------------------------------
    // --- Light primitives (#73) --------------------------------------------
    // Analytic light list at engine slot 11 (MSL slot order;
    // vk::binding(27, 0)). The shader gates its NEE-to-lights branch
    // on push.light_count > 0; the binding still has to resolve to a
    // real buffer for the same reason 4/5/7/8/9/10 do (Metal computes
    // dynamic push slots from max-bound + 1 of the contiguous range
    // -- leaving 11 unbound would shift the push slot and corrupt
    // every field). Fall back to the always-present placeholder
    // storage buffer when no lights are active.
    pt::rhi::BufferHandle slot11 = (light_buffer_id_ != 0)
    ...
    struct PtPush {
        ...
        // NEE-to-lights branch entirely. Reuses the slot that was
        // _pad_star_split[0]; same wire offset, struct size unchanged.
    ...
    // buffer for slot stability -- see slot11 above).
