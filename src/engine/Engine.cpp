...
void Engine::TearDownDevice() {
    ...
    // --- end Light primitives ----------------------------------------------
    // --- Fluid Phase 1 (#136) ---------------------------------------------
    // Buffer is recreated lazily on the next render-frame upload.
    smoke_buffer_id_       = 0;
...
void Engine::RenderFrame() {
    ...
    // --- end Light primitives ----------------------------------------------
    // --- Fluid Phase 1 (#136) -- smoke emitters ----------------------------
    // Smoke emitter list at engine slot 13 (MSL slot order;
    // vk::binding(28, 0)). Declared AFTER light_prims in PathTrace.slang
    // so MSL slot ordering puts it one beyond slot 12. Always-bound
    // (placeholder fallback) for the same max-bound + 1 push-slot rule
    // slots 10/11/12 already follow. The shader gates its emitter loop
    // on push.smoke_count > 0 AND push.smoke_enabled != 0, so reading a
    // placeholder is dead at runtime when no emitters are active.
    pt::rhi::BufferHandle slot13 = (smoke_buffer_id_ != 0)
    ...
    struct PtPush {
        ...
        // --- end Water Phase 1 -------------------------------------------
        // --- Fluid Phase 1 (#136) ----------------------------------------
        // smoke_params.x = smoke_count (uint-as-float, number of emitters
        //                  the shader should iterate; capped to
        //                  kMaxSmokeEmitters host-side).
        //             .y = smoke_enabled (uint-as-float 0/1; the shader
        //                  short-circuits the whole loop when 0 so
        //                  r_smoke_enabled 0 is bit-exact vs pre-#136).
        //             .z = time_seconds (shared accumulator with cloud
        //                  wind / water waves; wraps on 24 h sim window).
        //                  Drives parametric drift: emitter pos =
        //                  base + velocity * time.
        //             .w = reserved (future: per-frame velocity scale,
        //                  global density boost, etc.).
