    ...
    vkResetCommandBuffer(pcb, 0);
    {
        ...
        if (swap_img != VK_NULL_HANDLE) {
            ...
            if (dst_view != VK_NULL_HANDLE && swap_view != VK_NULL_HANDLE &&
                ...
                // Stars binding retired with the EMA design (#108).
                // Pass VK_NULL_HANDLE for stars_view -- DenoiseFinalize
                // no longer reads stars; the parameter is kept on the
                // signature so OptiX/SVGF call sites stay symmetric in
                // this PR.
